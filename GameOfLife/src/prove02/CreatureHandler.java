...
/*	static boolean testForSpawn = false;
	static  Wolves babyCreature;
*/
	/**
...
//		for(Creature c : _creatures) {
			...
			if(c instanceof Aware) {
				...
				if(c instanceof Aggressor) {
					Creature target = getTarget(c, 0, 0); //returns target if there is any otherwise null
					Aggressor a = (Aggressor)c;
					boolean testForSpawn =  a.attack(target); // return true if attacker or 'a' is wolf



					//I added testForSpan and made it return true or fasle based on attack result +
					  // I created static creature babycreature so it can be accessed by game class.
					if (testForSpawn == true) {

						Wolves babyCreature = new Wolves();


						babyCreature.setLocation(new Point((int) c.getLocation().getX() - 1, (int) c.getLocation().getY()));
						 _creatures.add(babyCreature);
						System.out.println("new creature createdddddddddddddddddddddddddddddddddddddddddddddddbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbvvsjbbbbsbdvdjd");
						System.out.println("new creature createdddddddddddddddddddddddddddddddddddddddddddddddbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbvvsjbbbbsbdvdjd");
						System.out.println("new creature createdddddddddddddddddddddddddddddddddddddddddddddddbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbvvsjbbbbsbdvdjd");
					...
					//just have to create a wolf creature  and add in a list
